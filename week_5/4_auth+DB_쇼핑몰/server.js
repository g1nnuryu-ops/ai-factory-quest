...
// .env 로드: 앱 로컬(JWT_SECRET, PORT) → week_5 공유 .env(DATABASE_URL 여기 있음) 순서.
// dotenv 는 이미 설정된 값을 덮어쓰지 않으므로 로컬 .env 가 우선한다.
...
// 내 주문/결제 내역(마이페이지). 완료(paid) 건을 최신순으로. 미결제(pending) 는 숨긴다.
